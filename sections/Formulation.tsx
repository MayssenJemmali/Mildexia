...
import { Dna, Leaf, Droplets, Sparkles, Layers, Waves, FlaskConical, Sprout } from 'lucide-react';

interface FormulationProps {
  content: Content['formulation'];
}

const Formulation: React.FC<FormulationProps> = ({ content }) => {
  
  const icons = [
    <Sprout size={20} />,       // Prêle (Horsetail)
    <FlaskConical size={20} />, // Ail (Garlic)
    <Leaf size={20} />,         // Neem
    <Droplets size={20} />,     // Romarin (Rosemary Oil)
    <Sparkles size={20} />,     // Bicarbonate
    <Layers size={20} />,       // Kaolin
    <Waves size={20} />         // Algues (Seaweed)
  ];

  return (
    <section className="py-32 relative overflow-hidden bg-gradient-to-b from-mildexia-cream to-white dark:from-mildexia-deep dark:to-black">
      <div className="container mx-auto px-6 text-center relative z-10">
        ...
        <div className="relative w-full max-w-3xl mx-auto aspect-square hidden lg:block">
          {/* Center Core */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-white dark:bg-mildexia-charcoal rounded-full shadow-[0_0_60px_rgba(16,185,129,0.2)] flex flex-col items-center justify-center z-20 border border-mildexia-primary/20">
            <div className="p-3 bg-mildexia-primary/10 rounded-full mb-2">
                <Leaf className="text-mildexia-primary" size={32} />
            </div>
            <span className="font-display font-bold text-3xl text-mildexia-primary">Mildexia</span>
          ...
          {content.ingredients.map((item, index) => {
            ...
            return (
              <motion.div
                key={index}
                className="absolute w-40 -ml-20 -mt-8 flex flex-col items-center gap-2"
                style={{ left: `${x}%`, top: `${y}%` }}
              ...
              >
                <div className="w-12 h-12 bg-white dark:bg-mildexia-charcoal rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 flex items-center justify-center text-mildexia-primary dark:text-mildexia-neon">
